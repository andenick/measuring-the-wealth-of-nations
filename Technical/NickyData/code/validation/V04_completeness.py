...
def validate(series_filter=None, chapter_filter=None):
    ...
    # Handle both series_registry.json (flat) and project_registry.json (nested)
    series_dict = registry.get("series", {})
    if not series_dict and "book_replication" in registry:
        series_dict = {}
        for ch_data in registry["book_replication"].get("chapters", {}).values():
            for sid in ch_data.get("series", []):
                series_dict[sid] = {"wave": 1}
    ...
    for csv_path in series_files:
        ...
        entry = series_dict.get(series_id, {})
        if chapter_filter is not None and entry.get("chapter") != chapter_filter:
            continue

        wave = entry.get("wave", 1)  # Default to wave 1 for T-series
        if wave > 1:
