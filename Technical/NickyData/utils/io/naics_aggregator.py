...
def compute_marxian_aggregates(go_data: pd.DataFrame) -> pd.DataFrame:
    ...
    # Add derived columns
    total = result["GO_productive"] + result["GO_trading"] + result["GO_unproductive"] + result["GO_govt_enterprise"] + result["GO_govt_admin"]
    result["productive_share"] = (result["GO_productive"] / total).round(4)
    result["trading_share"] = (result["GO_trading"] / total).round(4)
    result["TV_star_growth"] = result["TV_star"].pct_change().round(4)
