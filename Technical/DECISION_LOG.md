...
## DEC-012: Archive nickydata/ — Single Canonical Pipeline (2026-05-11)

**Decision**: Archive `nickydata/` v7.2 to `_archive/nickydata_v7.2_2026-05-11/` and maintain a single canonical pipeline in `code/`. All innovations (412-industry detail IO, 3-era V*, depreciation, period analysis) migrated to code/ scripts (L19, L20, P22, A11, updated P03/P07/P14).

**Rationale**: Two parallel implementations created maintenance burden and confusion. The code/ pipeline has 76 scripts, full Anu Suite artifacts, and 15 validators. nickydata/ was a lean research prototype that proved the methodology but lacked infrastructure.

**Impact**: Single pipeline to maintain. nickydata/ preserved in archive for reference.

---

## DEC-013: K* from Industry-Level Fixed Assets (2026-05-11)

**Decision**: Compute K* = Σ K_j for productive + trading sectors using BEA FAAt401 by industry (L20), replacing the total-K × productive_ratio approximation. P08 uses K*-based r* = S*/K* when K_star_by_industry.csv is available.

**Rationale**: The book restricts capital stock to productive sectors. Total K includes FIRE and real estate capital which is unproductive by definition. Industry-level data provides the correct decomposition.

**Impact**: T513/T514 profit rates now use productive-sector K*. DIV-001 resolved.

---

## DEC-014: NAICS Labor Values via Approximate hp* (2026-05-11)

**Decision**: Extend T701-T703 to NAICS era (1997-2017) using NIPA 6.5D FTE distributed to IO sectors as approximate hp* vectors, combined with NAICS Leontief inverses from L11b. Ochoa R² = 0.85-0.99 for NAICS benchmarks.

**Rationale**: Exact sector-level employment-to-IO mapping requires a concordance that doesn't exist at full detail. The NIPA 6.5D → IO mapping is approximate but produces strong value-price correlations, confirming the labor theory of value holds for the NAICS era.

**Known limitation**: 1997 uses uniform hp* (NIPA 6.5D starts at 1998). r_bar computation needs sector-level GO from the Use table rather than A-matrix column sums.

---

## DEC-015: T510 Extension via K*/V* Components (2026-05-11)

**Decision**: Extend T510 (C*/V* = value composition of capital) using K*/V* from industry Fixed Assets (K* from L20) and V* from P02, replacing the linear trend extrapolation.

**Rationale**: Linear trend extrapolation has no economic content. K*/V* from components captures the actual trajectory of capital deepening in productive sectors.

---

*Last updated: 2026-05-11 (Session 26 — consolidation)*
