...
import pandas as pd
import numpy as np
from lib.paths import SERIES_OUT, INPUTS, ensure_dirs
...
def process():
    ...
    out.index.name = "year"
    out.to_csv(SERIES_OUT / "N1001.csv")
    data_dict["N1001"] = labor_share
    outputs.append(str(SERIES_OUT / "N1001.csv"))
    ...
    out.index.name = "year"
    out.to_csv(SERIES_OUT / "N1002.csv")
    data_dict["N1002"] = net_tax
    outputs.append(str(SERIES_OUT / "N1002.csv"))
    ...
    if mohun_emp_path.exists():
        ...
        if "year" in mohun.columns and "Lu_mohun" in mohun.columns:
            # N1501: Working class unproductive (approximate as 60% of Lu)
            lu = mohun.set_index("year")["Lu_mohun"].dropna()
            luw = lu * 0.60  # Working class unproductive
            lum = lu * 0.40  # Managerial unproductive
            ...
            for sid, series, desc in [
                ...
                out.index.name = "year"
                out.to_csv(SERIES_OUT / f"{sid}.csv")
                data_dict[sid] = series
                outputs.append(str(SERIES_OUT / f"{sid}.csv"))
            ...
            if "Lp_mohun" in mohun.columns:
                ...
                out.index.name = "year"
                out.to_csv(SERIES_OUT / "N1504.csv")
                data_dict["N1504"] = burden
                outputs.append(str(SERIES_OUT / "N1504.csv"))
    ...
    out.index.name = "year"
    out.to_csv(SERIES_OUT / "N1701.csv")
    data_dict["N1701"] = nz_prod_share
    outputs.append(str(SERIES_OUT / "N1701.csv"))
