    ...
    for sid in SERIES_IDS:
        ...
        if t504 is not None and t505 is not None:
            ...
            if sid == "T507":
                ...
                # T510 = C* / V* — need C*m from T502
                # C*m is in billions (Table E.2) but V* is in millions — can't compute directly
                # Use the book-period ratio and extend via trend
                # For now: just use book data (T510 extension blocked by unit mismatch)
                ext = pd.Series(dtype=float)
                steps.append(f"T510: extension blocked by C*/V* unit mismatch (C* in billions, V* in millions)")
            else:
                ext = pd.Series(dtype=float)
