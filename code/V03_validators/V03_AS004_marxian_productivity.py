"""V03_AS004 — productivity must rise over book period (Marxian q* growth).
...
def run():
    df = pd.read_csv(DATA_FINAL / "AS004.csv")
    df = df[df["series_id"] == "AS004-A"].sort_values("year")
    ...
    bench = BenchmarkValidator(
        series_id="AS004",
        tolerance_class=get_tolerance_class("AS004", default="rate_series"),
        benchmarks=get_reference_values("AS004"),
        subseries_filter="AS004-A",
    ).run(DATA_FINAL / "AS004.csv")
    ...
    write_validation_result("AS004", result)
    print(f"    [V03_AS004] status={status} q*: {df.iloc[0]['year']}={first_v:.2f} -> {df.iloc[-1]['year']}={last_v:.2f} ({growth_pct:+.1f}%); bench={bench['n_pass']}/{bench['n_pass']+bench['n_fail']+bench['n_missing']}")
    return result
