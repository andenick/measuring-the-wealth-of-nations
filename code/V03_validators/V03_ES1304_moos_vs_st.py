"""V03_ES1304 — Moos vs ST range check + registry benchmark check.
...
def run():
    df = pd.read_csv(DATA_FINAL / "ES1304.csv")
    df = df[df["series_id"] == "ES1304-A"]
    in_range = bool(df["value"].between(-0.10, 0.10).all())

    bench = BenchmarkValidator(
        series_id="ES1304",
        tolerance_class=get_tolerance_class("ES1304", default="share_series"),
        benchmarks=get_reference_values("ES1304"),
        subseries_filter="ES1304-A",
    ).run(DATA_FINAL / "ES1304.csv")
    ...
    write_validation_result("ES1304", result)
    print(f"    [V03_ES1304] status={status} range=[{df['value'].min():.4f}, {df['value'].max():.4f}]; bench={bench['n_pass']}/{bench['n_pass']+bench['n_fail']+bench['n_missing']}")
    return result
