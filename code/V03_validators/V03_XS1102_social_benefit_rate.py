"""V03_XS1102 — Range check + registry benchmark for Social Benefit Rate (S&T 1987).
...
def run():
    df = pd.read_csv(DATA_FINAL / "XS1102.csv")
    df = df[df["series_id"] == "XS1102-A"]
    in_range = bool(df["value"].between(-1.0, 1.0).all())

    bench = BenchmarkValidator(
        series_id="XS1102",
        tolerance_class=get_tolerance_class("XS1102", default="share_series"),
        benchmarks=get_reference_values("XS1102"),
        subseries_filter="XS1102-A",
    ).run(DATA_FINAL / "XS1102.csv")
    ...
    write_validation_result("XS1102", result)
    print(f"    [V03_XS1102] status={status} range=[{df['value'].min():.4f}, {df['value'].max():.4f}]; bench={bench['n_pass']}/{bench['n_pass']+bench['n_fail']+bench['n_missing']}")
    return result
