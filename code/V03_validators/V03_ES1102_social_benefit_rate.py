"""V03_ES1102 — Range check + registry benchmark for Social Benefit Rate (S&T 1987).
...
def run():
    ...
    bench = BenchmarkValidator(
        series_id="ES1102",
        tolerance_class=get_tolerance_class("ES1102", default="share_series"),
        benchmarks=get_reference_values("ES1102"),
        subseries_filter="ES1102-A",
    ).run(DATA_FINAL / "ES1102.csv")
    ...
    print(f"    [V03_ES1102] status={status} range=[{df['value'].min():.4f}, {df['value'].max():.4f}]; bench={bench['n_pass']}/{bench['n_pass']+bench['n_fail']+bench['n_missing']}")
    return result
