"""V03_ES1301 — range check for Moos NSW/GDP."""
...
def run():
    df = pd.read_csv(DATA_FINAL / "ES1301.csv")
    df = df[df["series_id"] == "ES1301-A"]
    ...
    write_validation_result("ES1301", result)
    print(f"    [V03_ES1301] status={status} range=[{df['value'].min():.4f}, {df['value'].max():.4f}]")
    return result
