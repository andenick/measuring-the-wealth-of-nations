...
def run():
    df = pd.read_csv(DATA_FINAL / "ES1601.csv")
    df = df[df["series_id"] == "ES1601-A"]
    ...
    result = {
        ...
        "range_check": {"expected": [0.2, 0.8], "actual": [float(df["value"].min()), float(df["value"].max())]},
    }
    write_validation_result("ES1601", result)
    print(f"    [V03_ES1601] status={status} range=[{df['value'].min():.4f}, {df['value'].max():.4f}]")
    return result
