"""V03_ES1202 — NSW/EC range check + registry benchmark check.
...
def run():
    ...
    bench = BenchmarkValidator(
        series_id="ES1202",
        tolerance_class=get_tolerance_class("ES1202", default="share_series"),
        benchmarks=get_reference_values("ES1202"),
        subseries_filter="ES1202-A",
    ).run(DATA_FINAL / "ES1202.csv")
    ...
    print(f"    [V03_ES1202] status={status} NSW/EC range=[{df['value'].min():.4f}, {df['value'].max():.4f}]; bench={bench['n_pass']}/{bench['n_pass']+bench['n_fail']+bench['n_missing']}")
    return result
