"""V03_S801 — Cross-study merge round-trip + registry benchmarks.
...
Upgraded from round-trip-only to also check year-by-year reference values.
"""
...
def run():
    ...
    round_trip_pass = (max_err < 1e-9 and len(df) >= 30)

    # Registry benchmark check (per Decision 0002)
    benchmarks = get_reference_values("S801")
    ...
    status = "PASS" if (round_trip_pass and n_bench_fail == 0 and n_bench_miss == 0) else "FAIL"
    result = {
        "series_id": "S801",
    ...
    print(f"    [V03_S801] status={status} round_trip_max_err={max_err} bench_pass={n_bench_pass}/{n_bench_pass+n_bench_fail+n_bench_miss}")
    return result
