"""V03_XS1201 — NSW/GDP range check + registry benchmark check (ST 2002).
...
def run():
    df = pd.read_csv(DATA_FINAL / "XS1201.csv")
    df = df[df["series_id"] == "XS1201-A"]
    in_range = bool(df["value"].between(-0.05, 0.10).all())

    bench = BenchmarkValidator(
        series_id="XS1201",
        tolerance_class=get_tolerance_class("XS1201", default="share_series"),
        benchmarks=get_reference_values("XS1201"),
        subseries_filter="XS1201-A",
    ).run(DATA_FINAL / "XS1201.csv")
    ...
    result = {
        ...
        "range_check": {"expected": [-0.05, 0.10],
                        "actual": [float(df["value"].min()), float(df["value"].max())]},
        "benchmarks": bench.get("benchmarks", {}),
    }
    write_validation_result("XS1201", result)
    print(f"    [V03_XS1201] status={status} NSW/GDP range=[{df['value'].min():.4f}, {df['value'].max():.4f}]; bench={bench['n_pass']}/{bench['n_pass']+bench['n_fail']+bench['n_missing']}")
    return result
