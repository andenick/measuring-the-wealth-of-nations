"""V03_ES1103 — Range check + registry benchmark for Social Tax Rate (S&T 1987).
...
def run():
    df = pd.read_csv(DATA_FINAL / "ES1103.csv")
    df = df[df["series_id"] == "ES1103-A"]
    in_range = bool(df["value"].between(-1.0, 1.0).all())

    bench = BenchmarkValidator(
        series_id="ES1103",
        tolerance_class=get_tolerance_class("ES1103", default="share_series"),
        benchmarks=get_reference_values("ES1103"),
        subseries_filter="ES1103-A",
    ).run(DATA_FINAL / "ES1103.csv")
    ...
    write_validation_result("ES1103", result)
    print(f"    [V03_ES1103] status={status} range=[{df['value'].min():.4f}, {df['value'].max():.4f}]; bench={bench['n_pass']}/{bench['n_pass']+bench['n_fail']+bench['n_missing']}")
    return result
