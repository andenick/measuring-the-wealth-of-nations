"""P02_ES1102 — Social Benefit Rate (S&T 1987) (derived ratio).

Computed as (S605 + S606) / S617.
...
def compute() -> pd.DataFrame:
    ...
    S606 = _load('S606')
    S617 = _load('S617')
    merged = S605.merge(S606, on='year').merge(S617, on='year').sort_values('year').reset_index(drop=True)
    merged["value"] = (merged['S605'] + merged['S606']) / merged['S617']
    merged["series_id"] = SUBSERIES
    merged["units"] = "share"
    merged["stage"] = "analytical_derivation"
    merged["provenance"] = "(S605 + S606) / S617"
...
def run():
    ...
    print(f"    [P02_ES1102] {len(df)} rows; first={df.iloc[0]['value']:.4f}, last={df.iloc[-1]['value']:.4f}; wrote {final_path.name}")
    return df
