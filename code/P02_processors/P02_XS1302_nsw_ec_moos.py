"""P02_XS1302 — Moos NSW / compensation (Moos's own compensation column)."""
...
def run():
    ...
    print(f"    [P02_XS1302] {len(df)} rows; first={df.iloc[0]['value']:.4f}, last={df.iloc[-1]['value']:.4f}; wrote {final_path.name}")
    return df
