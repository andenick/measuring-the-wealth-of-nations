"""P02_ES1304 — Moos vs ST NSW/GDP comparison at overlap years 1959-1997.
...
def compute() -> pd.DataFrame:
    es1201 = pd.read_csv(DATA_FINAL / "ES1201.csv")
    es1201 = es1201[es1201["series_id"] == "ES1201-A"][["year", "value"]].rename(columns={"value": "ST_NSW_GDP"})
    es1301 = pd.read_csv(DATA_FINAL / "ES1301.csv")
    es1301 = es1301[es1301["series_id"] == "ES1301-A"][["year", "value"]].rename(columns={"value": "Moos_NSW_GDP"})
    ...
    merged["provenance"] = "ES1301 - ES1201"
...
def run():
    ...
    print(f"    [P02_ES1304] {len(df)} overlap years; delta range=[{df['value'].min():.4f}, {df['value'].max():.4f}]; wrote {final_path.name}")
    return df
