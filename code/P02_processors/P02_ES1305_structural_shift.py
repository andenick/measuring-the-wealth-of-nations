"""P02_ES1305 — Post-2000 Structural Shift Indicator (Moos 2017).
...
def compute() -> pd.DataFrame:
    ...
    es1301 = es1301[es1301["series_id"] == "ES1301-A"][["year", "value"]].rename(columns={"value": "NSW_GDP"})
    ...
    es1301["provenance"] = f"ES1301 - mean(ES1301[1980-1999]={pre2000:.4f})"
...
def run():
    ...
    print(f"    [P02_ES1305] {len(df)} rows; post-2000 delta range=[{df[df['year']>=2000]['value'].min():.4f}, {df[df['year']>=2000]['value'].max():.4f}]; wrote {final_path.name}")
    return df
