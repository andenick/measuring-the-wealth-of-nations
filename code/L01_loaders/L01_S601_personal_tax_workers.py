"""L01_S601 — Load Personal Tax Workers, book period (Ch6 NSW).

Source: Appendix Ch6 — `data/source/book_tables/Table6_1_TaxAccounts.csv`, column
`personal_income_tax_workers`. Source values in MILLIONS; loader divides by 1000 for BILLIONS output, matching the registry's `units: billions_usd`.
...
LOADER = BookColumnLoader(
    ...
    source_column = "personal_income_tax_workers",
    units         = "billions_usd",
    unit_scale    = 1000.0,
)
...
def run():
    df = LOADER.load()
    print(f"    [L01_S601] loaded {len(df)} rows; "
