"""L01_S605 — Load Government Benefits to Workers (B_w), book period (Ch6 NSW).

Source: Appendix Ch6 — `data/source/book_tables/Table6_2_BenefitAccounts.csv`, column
`total_benefits`. Source values in MILLIONS; loader divides by 1000 for BILLIONS output, matching the registry's `units: billions_usd`.
...
def run():
    df = LOADER.load()
    print(f"    [L01_S605] loaded {len(df)} rows; "
