...
1. **BLS Current Employment Statistics (CES)** — production-worker employment counts `(Lp)_j` and average weekly hours `(h)_j` per supersector, 1948–2024 where available (mining + construction + manufacturing have full 1948+ coverage; services 1964+; trade subsectors 1972+). Cached at `data/raw/bls/bls_ces_<supersector_slug>_employment.csv` and `bls_ces_<supersector_slug>_hours.csv` / `_hours_alt.csv`, pulled 2026-05-24 via `Technical/code/L00_setup/L00_bls_fetch.py`. Provenance: `data/raw/bls/bls_ces_fetch_provenance.json`. BLS API key supplied via the `BLS_API_KEY` environment variable or a local `api_keys.env`.

2. **BEA gross output `X_j`** — NIPA Table 1.7.5 (Gross Output by Industry, annual 1947+) and BEA Industry GDP-by-Industry (NAICS, 1997+). Cached at `data/raw/bea/nipa_1_7_5_gross_output_by_industry.csv` and `gdp_by_industry_gross_output.csv`, pulled 2026-02-24 via `pull_bea_nipa_ch05.py`. Provenance: `data/raw/bea/provenance.json`.

3. **Labeled BEA IO matrices** — the `(I - A*)^{-1}` Leontief inverse, built from labeled BEA benchmark IO tables (1947, 1958, 1963, 1967, 1972, 1977 SIC; 1997, 2002, 2007, 2012 NAICS). Cached at `Technical/data/intermediate/io_matrices_labeled/`, derived from `data/raw/io_matrices/`. Appendix F productive-share filter `Technical/data/source/appendix_F/Table_F_1.csv` is applied to mask unproductive sectors before computing the Leontief inverse on the productive sub-matrix.
