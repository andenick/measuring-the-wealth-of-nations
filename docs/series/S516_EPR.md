# S516 — Extension Provenance Record

**Series**: Unproductive Employment (Lu = L - Lp)
**Units**: thousands
**Book period**: 1948–1989 (canonical, in `data/final/S516.csv`)
...
- **Dependencies on other series**: S515
...
- `S516-EXT` — derived (period [1990, 2024], units thousands)
- `S516-COMBINED` —  (period [1948, 2024], units thousands)

## Activation criteria

Before the extension is fetched and spliced into `data/final/S516.csv`:
...
Per the Anu Extension Standard (no proxies, no lazy splices on derived quantities): the extension MUST use the agency/table the book originally drew from, or document any substitution explicitly. For S516, the canonical BEA/BLS/FRED endpoint is recorded in the registry under `subseries[S###-B].source`. Any divergence requires a Concept Match Justification in this EPR.
