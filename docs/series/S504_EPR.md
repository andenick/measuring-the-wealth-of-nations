...
**Units**: billions_usd
**Book period**: 1948–1989 (canonical, in `data/final/S504.csv`)
...
- `S504-EXT` — BEA NIPA + BLS CES (period [1990, 2024], units billions_dollars)
- `S504-COMBINED` —  (period [1948, 2024], units billions_dollars)

## Activation criteria

Before the extension is fetched and spliced into `data/final/S504.csv`:
...
Per the Anu Extension Standard (no proxies, no lazy splices on derived quantities): the extension MUST use the agency/table the book originally drew from, or document any substitution explicitly. For S504, the canonical BEA/BLS/FRED endpoint is recorded in the registry under `subseries[S###-B].source`. Any divergence requires a Concept Match Justification in this EPR.
