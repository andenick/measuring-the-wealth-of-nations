# S506 — Extension Provenance Record

**Series**: Rate of Exploitation (e = S*/V*)
**Units**: ratio
**Book period**: 1948–1989 (canonical, in `data/final/S506.csv`)
...
- **Dependencies on other series**: S512
...
- `S506-EXT` — BLS CES + BEA NIPA (period [1990, 2024], units ratio)
- `S506-COMBINED` —  (period [1948, 2024], units ratio)

## Activation criteria

Before the extension is fetched and spliced into `data/final/S506.csv`:
...
Per the Anu Extension Standard (no proxies, no lazy splices on derived quantities): the extension MUST use the agency/table the book originally drew from, or document any substitution explicitly. For S506, the canonical BEA/BLS/FRED endpoint is recorded in the registry under `subseries[S###-B].source`. Any divergence requires a Concept Match Justification in this EPR.
